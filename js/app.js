...
function initTelegramWebApp() {
    ...
    tg.MainButton.show();
    tg.MainButton.onClick(() => requestBalanceUpdate());
...
function loadWalletDataFromUrl() {
    ...
    console.log('Данные из URL параметров:', {
        address: walletData.address,
        balance: walletData.balance,
        rate: exchangeRate.rate,
        rateUpdated: exchangeRate.updatedAt,
        isRateActual: exchangeRate.isActual,
    ...
    if (hasTransactions) {
        // Отправляем запрос на получение истории транзакций
...
// Запрос истории транзакций у бота
function requestTransactionHistory() {
    ...
    // Показываем пользователю промежуточное сообщение
    document.getElementById('transactions-container').innerHTML = `
        <div class="loading-indicator">
            <i class="fas fa-spinner fa-spin"></i>
            <p>Получение истории транзакций...</p>
        </div>
    `;
    
    // Отправляем данные в Telegram
    // Бот ответит через чат, не через WebApp, поэтому показываем сообщение
    setTimeout(() => {
        document.getElementById('transactions-container').innerHTML = `
            <div class="notification info">
                <p>История транзакций запрошена и будет отправлена в чате с ботом</p>
            </div>
        `;
    }, 2000);
}
...
function setupActionButtonHandlers() {
    ...
    // Обработчик для кнопки обновления баланса
    const refreshBalanceBtn = document.getElementById('refresh-button');
...
// Запрос обновления баланса у бота
function requestBalanceUpdate() {
    ...
    // Отправляем данные в Telegram
    sendDataToTelegram(balanceData);
    
    // Показываем сообщение об отправке запроса
    setTimeout(() => {
        hideLoadingState();
        showSuccess('Запрос на обновление баланса отправлен в бот');
    }, 1000);
} 